...
export const ProductoProvider = ({ children }) => {
    ...
    const loadProducto = async () => {
        try {
            ...
            try {
                const responseP = await Promise.allSettled(
                    response.data.result.map(async (prod) => {
                        var producto = {
                            idProducto: prod.idProducto,
                            nombre: prod.nombre,
                ...
                var product = isFulfilled(responseP)
            ...
            setProductos([...productos, product[0]])
        } catch (error) {
            console.error(error)
        }
    }
    ...
    return (
        <ProductoContext.Provider value={{ productos, getProducto, loadProductos, loadProducto }}>
            {children}
